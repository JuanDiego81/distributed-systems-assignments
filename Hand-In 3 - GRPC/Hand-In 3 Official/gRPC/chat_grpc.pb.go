...
func (c *chitChatClient) Chat(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ChatMessage, ChatMessage], error) {
...
type ChitChat_ChatClient = grpc.BidiStreamingClient[ChatMessage, ChatMessage]
...
func (UnimplementedChitChatServer) Chat(grpc.BidiStreamingServer[ChatMessage, ChatMessage]) error {
...
func _ChitChat_Chat_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ChitChatServer).Chat(&grpc.GenericServerStream[ChatMessage, ChatMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChitChat_ChatServer = grpc.BidiStreamingServer[ChatMessage, ChatMessage]
